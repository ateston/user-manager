...
public class Communication {
	...
	// Metodo que retorna true si logra conectarse a la red y false si no es
	// posible
	public boolean connectToSession() throws Exception {
		boolean flag = false;
		// Buscamos la senal de broadcast en los 20 puertos que soporta el
		// sistema
		...
		if (flag)
			System.out.println("Nodo " + id
					+ " te has conectado exitosamente a la red");
		else {
			nodos.add(id);
	...
	// MŽtodo que env’a la actual lista (actualizada) a todos los nodos que
	// estan en la lista
	public void updateEveryonesList() throws Exception {
		Iterator<Integer> i = nodos.iterator();
		while (i.hasNext()) {
			// Como enviamos la lista debe ser un mensaje de caracter update -->
			// 3
	...
	// MŽtodo que recibe la lista de nodos separados por comas, limpia la actual
	// lista y luego la actualiza con la nueva info
	public void updateList(String listaNodos) {
		nodos.clear();
		getListaNodos(listaNodos);
	}

	// MŽtodo que toma la lista de nodos (atributo) y lo transforma en un string
	// en donde
	...
	// MŽtodo que env’a mensajes. Recibe como input el string del mensaje que se
	// desea enviar y el id del nodo
	...
	public void sendMessage(String Message, int idDestino) throws Exception {
		...
		if (nodos.contains(idDestino)
				|| Message.substring(0, 1).compareTo("1") == 0) {
			try {
	...
	// MŽtodo que env’a objetos. Recibe como input el objeto que se desea enviar
	// y el id del nodo
	...
	// MŽtodo para desconectar el nodo de la red. Elimina el id del nodo de la
	// lista y luego
	...
	// MŽtodo principal de caracter runnable. En el main al hacer
	// "new Thread(c).start()" se abre
	// un nuevo thread que corre œnicamente este mŽtodo. Lo que hace es quedarse
	// en un ciclo infinito
	// con un servidor levantado que est‡ constantemente escuchando. Luego
	// investiga el mensaje recibido
				...
				if (!envioDeObjeto) {
					...
					try {
						if (Integer.parseInt(clientSentence.substring(0, 4)) != id) {
							// [Solicitud llegada] Llegada de un nuevo Nodo,
							// debemos responder su solicitud y enviar la actual
							// lista de nodos
							if (clientSentence.substring(5, 6).compareTo("0") == 0) {
								int nuevoID = Integer.parseInt(clientSentence
										.substring(0, 4));
								sendMessage("1" + "_" + listaNodosToString(),
										nuevoID); // 1 = Llegada Aceptada
								System.out.println("Se ha agregado el nodo "
										+ nuevoID + " a la red");
							}
							// [Llegada aceptada] Me responden la solicitud de
							// llegada
							if (clientSentence.substring(5, 6).compareTo("1") == 0) {
								System.out.println("Te has unido a "
										+ clientSentence.substring(0, 4));
								getListaNodos(clientSentence.substring(7));
								nodos.add(id);
								updateEveryonesList();
							}
							// [Mensaje normal] Lo imprime en consola
							else if (clientSentence.substring(5, 6).compareTo(
									"2") == 0) {
								System.out.println("From "
										+ clientSentence.substring(0, 4) + ": "
										+ clientSentence.substring(7));
							}
