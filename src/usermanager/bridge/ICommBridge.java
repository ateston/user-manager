...
public interface ICommBridge {
    ...
    // Desconectarse:
    // 1-Se llama a un metodo de comunicacion entregandole un string con el
    // username o un id del usuario
    // Comunication.disconectUser(currentUser)
    // Comunicacion se comunica con Comunicacion de los otros dispositivo
    // 2-Comunicacion(de los otros dispositvos) les comunican a sus respectivos
    // UserManagers que se desconecta el ususrio
    public void disconnectUser(String user);

    // Cambio en algun User
    // 1- UserManger notific� a Comunicacion de algun cambio ocurrido al User
    // del dispositivo, entregandole la instancia de este como Object
    // Comunication.currentUserChanged(currentUser)
    // 2- Comunicacion le env�a este Objeto al resto de los dispositivos
    // 3- Al llegar a algun dispositivo, Comunicacion llama a una metodo de User
    // Manager para que este actualize a este usuario
